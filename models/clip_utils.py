...
class CLIP(nn.Module):
    def __init__(self, model_name, local_files_only=False):
        ...
        # model_name: HuggingFace id or local dir, e.g. openai/clip-vit-base-patch32
        print(f'Initializing CLIP model from {model_name} ...')
        from transformers import CLIPProcessor, CLIPModel, CLIPTokenizerFast
        load_kw = {'local_files_only': local_files_only}
        self.model = CLIPModel.from_pretrained(model_name, **load_kw)
        self.model.eval()
        self.processor = CLIPProcessor.from_pretrained(model_name, **load_kw)
        self.tokenizer = CLIPTokenizerFast.from_pretrained(model_name, **load_kw)
